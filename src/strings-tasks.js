...
function findLongestWord(sentence) {
  ...
  const longestWord = splitStr.sort((a, b) => {
...
function invertCase(str) {
  const invert = str.split('').reduce((acc, char) => {
    return (
      acc +
...
function extractNameFromTemplate(value) {
  ...
  const lastName = words[2].slice(0, -1);
...
function getCardId(value) {
  ...
  const suit = Number(obj[value.slice(-1)]);
  const firstValue = value.slice(0, -1);
  const num = Number(firstValue) ? Number(firstValue) - 1 : obj[firstValue];
  return suit * 13 + num;
}
